<template>
  <div class="circulation-page">
    ...
    <!-- Dialog quét QR thẻ độc giả -->
    <v-dialog v-model="scanDialog" max-width="480">
      <v-card rounded="lg" class="pa-4">
        <v-card-title class="d-flex align-center justify-space-between pb-3">
          ...
          <v-btn icon="mdi-close" variant="text" size="small" @click="scanDialog = false" />
        </v-card-title>
...
async function handleQrScanned(cardNumber) {
  ...
  try {
    ...
    if (userDetails && userDetails.userId) {
      selectedReaderId.value = userDetails.userId
      success.value = true
      message.value = `Đã nhận diện độc giả: ${userDetails.fullName} (${userDetails.cardNumber})`
    } else {
      success.value = false
    ...
    success.value = false
    message.value = err.response?.data?.message || 'Không thể tra cứu thông tin thẻ'
